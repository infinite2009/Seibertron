...
export class ContainerWidgetComponent implements OnInit, OnDestroy {
  ...
  getStyles(index: number) {
    if (this.data?.children.length) {
      return this.basicFormService.convertSchemaToStyles(this.data.children[index].schema);
    }
    return {};
  }
   ...
   */
  output(key: string, index: number) {
    const { data, operation, state } = this.data?.children[index]?.schema?.dataMapping[key];
